...
本章では、はやる気持ちを少し抑えて、愛着を持ってもらえるスキルを作るにはどうしたらよいかを考えてみたいと思います。
...
## 意識しよう
### エンゲージメントとは
エンゲージメントとは、ざっくり言うと親近感・愛着・思い入れといった「つながり」のことです。@<br>{}

エンゲージメントを高めるには、”スキルを使う人はどんな人か”、”スキルを使ったら何がうれしいのか”を考えたうえでスキルを作る必要があります。
...
あまり深く考える必要はありません。  
専門的なことを知らなくてもスキル開発できるのがVoiceflowの強みですので、「ある人専用のスキルを作るために、ある人がどんな人かイメージを固める」というとらえ方で良いです。@<br>{}
...
たとえば「小学校の子どもが時間割を確認するスキル」を思いついたとします。  
”小学生”では抽象的ですので、もっとユーザーに寄せていきます。

### ペルソナの設定
ペルソナの設定をしてみましょう。  
具体的なユーザーが決まっていれば、そのユーザーをイメージして整理をします。  
整理する内容についてはさまざまな考え方がありますが、最初は分かっていることを並べるだけでもよいです。  
大事なのは、この時点で「情報の取捨選択をしない」ことです。どこでどう役立つか分かりませんので、のちの選択肢のためにユーザーのことが分かることは残しておきましょう。@<br>{}

ここでは、同じテーマであっても求められるものの違いが分かるように、二人の小学生のペルソナを書いてみます。

![長女と次女のペルソナ](images/chapxx-magistralla/persona1.png)

あっさりめですが、それぞれのペルソナに沿った時間割のスキルを考えていくと

- 長女はもうすぐ小学四年生であることから、自分で翌日の準備ができることを想像できるので「準備ができた後に確認をしてあげるスキル」
- 次女はまだ入学前であることから、一人で翌日の準備をすることに不慣れですので「一緒に準備を進めてあげるスキル」

が喜ばれそうです。@<br>{}

ユーザーについて整理をすると、同じテーマのスキルであっても求められるものに違い出ることがわかります。

### UIの検討
次にそのスキルのUIについて検討をします。  
VoiceUIありきで考え始めましたが、このタイミングで本当にその必要があるかを考えてください。  
その際に入力・出力の両方を音声で検討する必要はありません。既存のUIと組み合わせることが可能ですので、ユーザーの体験向上に一番効果の高いものを考えましょう。@<br>{}

![UIの組み合わせ](images/chapxx-magistralla/type1.png)

例題では、スキルを使う子どもたちの利用シーンを下記とイメージしました。
...
### 会話フローと内容
ペルソナを設定したことで状況や課題が明確になり、長女と次女で会話や内容を変える必要があることが分かりました。  
それぞれのペルソナに沿って会話フローと内容を考えます。
...
### Voiceflowでスキル作成
時間割というテーマと会話フローは同じですが、会話内容はそれぞれに特化した内容となっており、エンゲージメントが高くなっているといえます。  
...
## エンゲージメントを高めるために
### シノニム
人間どうしの会話では、無意識に「はい」と「YES」を同義語として認識しますが、アシスタントは同義語かどうかを判別してくれません。
...
### SSML
SSMLとは、Speech Synthesis Markup Language（合成音声マークアップ言語）と呼ばれ、会話の言葉を含めたさまざまな音声情報をXMLベースで表現したものです。  
...
「speak」ブロックに`audio`タグを入力します。Alexaスキルの場合、サウンドライブラリでタグのコピーが可能ですのでコピー&ペーストします。
...
Speechconとは、アシスタントが表情豊かに発話してくれる特別な言葉です。
