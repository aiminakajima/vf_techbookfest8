...
伊藤です。電子工作クラスタの人に「簡単に音声操作インタフェースをつける方法」を知っていただきたく、この章を書きました。
あるいはVUIクラスタの人も、もしまだでしたらM5StickC超楽しいですので```ぜひ軽率に電子工作へとハマりましょう。```
...
![採取した赤外線命令パターン](images/chapxx-sitopp/sito_024.png)

![コードへのコピー](images/chapxx-sitopp/sito_025.png)
...
* Mac上のファイルの保存場所を聞かれるので、適当に指定します。おすすめは、指定された場所にそのまま保存です。
* 書き込みにかかる時間、数十秒を待ちます。
...
* M5StickのLEDの横にある大きいボタン「M5ボタン」を押すと、エアコンがつきます。
...
AdafruitのMQTTライブラリを使います。また、Adafruit のMQTT Library をインストールするとついてくるMQTTのサンプルコードをアレンジして使いました。
